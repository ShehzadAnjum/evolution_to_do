...
class Task:
    ...
    Attributes:
        ...
        title: User-provided task title (required, non-empty)
...
|-------|------|----------|---------|-------------|-------------|
| id | int | Yes | Auto | Unique identifier | Sequential from 1, never reused |
| title | str | Yes | - | Task title | Non-empty after stripping whitespace |
| description | str \| None | No | None | Task details | Optional, can be empty string or None |
| completed | bool | Yes | False | Completion status | Boolean toggle |
...
- `description`: Optional, None or string (empty allowed)
...
- Per FR-005: "System MUST allow toggling task completion status"
...
| FR-013 | TaskStore.count(), count_completed(), count_remaining() |

## Reusability Notes (Principle IX)

The Task dataclass and TaskStore pattern are designed for extraction:

| Component | Reuse Path | Modifications for Phase II |
|-----------|------------|---------------------------|
| Task | Keep as base, extend | Add created_at, updated_at, user_id |
| TaskStore | Abstract to interface | Implement DatabaseTaskStore |

**Extraction Location**: `backend/src/lib/models/task.py` and `backend/src/lib/stores/base_store.py`
